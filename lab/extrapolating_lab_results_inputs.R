...
for (i in 2:sim_length){
  ...
  if (i %% 365 ==0){time_series$c.remaining[i]=time_series$c.remaining[i]+initial_c}
  ...
  if (i %% 365 ==0){time_series$c.remaining.hot[i]=time_series$c.remaining.hot[i]+initial_c}
...
tiff(filename="./figures/extrapolating_results_inputs.tiff",units="in",res=800,width=9,height=6,compression="lzw")
extrapolation
dev.off()
